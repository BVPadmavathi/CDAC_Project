...
uint8_t data1[sizeof(struct send)] = {0};
//hcsr04_data_t HCSR04_sensor;
//TaskHandle_t task1;

// tx and  rx structure
static CAN_TxHeaderTypeDef MyTxMssg;
...
void Task1b(void *p) {
	...
	for(;;)
		{
			Read_3Axis(data.axis);
			Ultrasonic(data.dx_cm);
...
void Task2(void *p)
{
	...
	for(;;)
	{
		xQueueReceive(Q2[0][0], &data, portMAX_DELAY);
...
//	char bufCAN[8] = {10, 20, 30, 40};
	for(;;)
	{
		HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_13);
		xQueueReceive(Q3, buf, portMAX_DELAY);
		//send to CAN
		HAL_UART_Transmit(&huart3, (uint8_t *)&data, sizeof(struct send), portMAX_DELAY);
//		if (HAL_CAN_AddTxMessage(&hcan1, &MyTxMssg, (uint8_t *)&data, &TxMailbox) != HAL_OK) {
//			Error_Handler();
//		}
//		while(HAL_CAN_IsTxMessagePending(&hcan1, TxMailbox));
//		HAL_CAN_AbortTxRequest(&hcan1, TxMailbox);
//	for(unsigned char i = 0; i < sizeof(struct send); i++) {
//		if (HAL_CAN_AddTxMessage(&hcan1, &MyTxMssg, (uint8_t *)&data + i, &TxMailbox) == HAL_OK) {
//			while(HAL_CAN_IsTxMessagePending(&hcan1, TxMailbox));
...
//		else
//			data1[i] = 0;
//		if (HAL_CAN_AddTxMessage(&hcan1, &MyTxMssg, (uint8_t *)&data1 + i, &TxMailbox) == HAL_OK) {
//			while(HAL_CAN_IsTxMessagePending(&hcan1, TxMailbox));
//			HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
//		}
//	}
...
int main(void)
{
  ...
  HAL_CAN_Start(&hcan1);
	...
	MyTxMssg.StdId = OwnID;
	MyTxMssg.RTR = CAN_RTR_DATA;
	MyTxMssg.IDE = CAN_ID_STD;
	MyTxMssg.DLC = sizeof(struct send);//1;//100;
////////////////////
...
//  HAL_TIM_Base_Start_IT(&htim6);
  ...
  static QueueHandle_t T1T2;
  	...
  	T1T2 = xQueueCreate(1, sizeof(struct send));
